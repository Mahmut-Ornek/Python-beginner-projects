...
def upscale_image(input_path, output_path, scale_factor):
    ...
    img = imagg.open(input_path)
    original_width, original_height = img.size

    print("Size of the PNG just before upscaling: ",original_width,"x",original_height)

    new_width = int(original_width * scale_factor)
    new_height = int(original_height * scale_factor)
    try:
        upscaled_img = img.resize((new_width, new_height), imagg.Resampling.LANCZOS)
    except AttributeError:
        upscaled_img = img.resize((new_width, new_height), imagg.LANCZOS)  # For older Pillow versions
    upscaled_img.save(output_path)
    print(f"Image successfully upscaled and saved to {output_path}") 

    print("\n\nSize of the PNG after upscaling: ",new_width,"x",new_height)

    total_pixels = new_width * new_height
    memory_bytes = total_pixels * channels
...
if __name__ == '__main__':
    ...
    # Example usage
    upscale_image("C:\\Users\\mahmu\\OneDrive\\Masaüstü\\Python Projects\\pyphotoshop-main\\input\\city.png", 
              "C:\\Users\\mahmu\\OneDrive\\Masaüstü\\Python Projects\\pyphotoshop-main\\output\\upscaled_img.png", 
              100)  # Scale factor 2 means doubling the dimensions
